...
<div class="mb-4">
    <a href="/tutorial/<?= $tutorial['id'] ?>/detail/create_detail" class="btn btn-success me-2">+ Tambah Detail
        Tutorial</a>
</div>
            ...
            <tr>
                            ...
                            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="codeModalLabel<?= $detail['id'] ?>">Code (Urutan
                                            <?= esc($detail['tutor_order']) ?>)</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <pre class="text-start"
                                            style="white-space: pre-wrap; font-size: 13px; font-family: monospace; background:#f8f9fa; color:#222; border:1px solid #ccc; padding:10px; border-radius:5px;"><?= esc($detail['code']) ?></pre>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <td class="text-center">
                    <?php if (!empty($detail['url'])): ?>
                        <a href="<?= esc($detail['url']) ?>" target="_blank" class="btn btn-outline-dark btn-sm">
                            Tampilkan URL
                        </a>
                    <?php else: ?>
                        <em class="text-muted">Tidak ada</em>
                ...
                <td class="text-center">
                    ...
                    <form action="/tutorial/<?= $tutorial['id'] ?>/detail/<?= $detail['id'] ?>/delete" method="post"
...
<?php if (empty($details)): ?>
    <p class="color:#fff; font-weight:600;">Belum ada detail tutorial.</p>
<?php endif; ?>
